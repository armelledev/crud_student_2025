...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create'])){
    ...
    if(empty($nom)){
        ...
        $query="SELECT * FROM students WHERE nom_student=? ";
...
if(empty($email)){
    ...
    $error['email_student']= "l'email nes pas valider";
        ...
        $query="SELECT * FROM students WHERE email_student=? ";
        $req =$pdoconnect->prepare($query);
        $req->execute([$email]);
        $student=$req->fetch();
        if($student){
...
if(empty($password)){
    $error['password_student'] ="le mot de passe est obligatoire";

}elseif (strlen($password) < 8) {
 $error['password_student'] = "le mot de passe doit contenir au moins 8 caracteres";

}elseif($password !== $password_confirm ){
     $error['password_student'] ="le mot de passe ne correspond pas";
}

//si aucune erreur, insert into database
if(empty($error)){

    $sql="INSERT INTO students (nom_student, email_student, password_student)
    VALUES(:nom_student, :email_student, :password_student)";
